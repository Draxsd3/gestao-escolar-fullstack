...
export default function AnoLetivo() {
  ...
  const fetch = () => {
    setLoading(true)
    api.get('/ano-letivo').then(r => setItems(r.data.data || r.data || [])).catch(()=>setItems([])).finally(()=>setLoading(false))
  }
  useEffect(()=>fetch(),[])

  const save = async e => {
    e.preventDefault(); setSaving(true); setError('')
    try { await api.post('/ano-letivo', form); setModal(false); setForm({nome:'',descricao:''}); fetch() }
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <div className="page-title">Ano Letivo</div>
        </div>
